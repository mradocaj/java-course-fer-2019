...
/**
 * Razred koji modelira naredbu za ispisivanje sadržaja danog direktorija (nerekurzivno).
 * Naredba prima jedan argument. Detaljniji opis naredbe može se dohvatiti pomoću metode 
...
public class LsShellCommand implements ShellCommand {		
	...
	public LsShellCommand() {
		...
		commandDescription.add("Second column contains object size in bytes that is right aligned "
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		Path path;
		try{
			path = env.getCurrentDirectory().resolve(Paths.get(argumentsList.get(0)));
	...
	/**
	 * Pomoćna metoda koja vraća liniju ispisa koja sadrži podatke o predanoj datoteci ili direktoriju.
	 * 
	...
	private String getLine(Path current) throws IOException {
		...
		BasicFileAttributeView faView = Files.getFileAttributeView(current, BasicFileAttributeView.class, 
