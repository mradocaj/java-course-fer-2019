...
/**
 * Razred koji modelira naredbu za ispisivanje sadržaja dokumenta.
...
public class CatShellCommand implements ShellCommand {		
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		Charset charset;
		try {
			charset = argumentsList.size() == 2 ? Charset.forName(argumentsList.get(1)) : Charset.defaultCharset();
		} catch (IllegalCharsetNameException | UnsupportedCharsetException ex) {
			env.writeln("Given charset cannot be resolved. \nIf path contains whitespaces, it must be written "
					+ "within quotation marks (\"C:\\home\\file.txt\").");
			return ShellStatus.CONTINUE;
		}
