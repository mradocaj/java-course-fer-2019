...
public class SmartScriptLexer {
	...
	/**
	 * Pomoćna metoda koja stvara novi token koji sadrži broj koji se nalazi unutar
	 * tag-a. Broj može biti double ili integer.
	 */
	...
	/**
	 * Pomoćna metoda koja ovisno o ulaznim znakovima stvara novi token određenog
	 * tipa.
	 */
	private void getNewToken() {
		if(currentIndex + 3 < data.length && Character.toUpperCase(data[currentIndex]) == 'F'
			...
			token = new SmartToken(SmartTokenType.END, "END");
				...
				&& Character.isDigit(data[currentIndex + 1]) || Character.isDigit(data[currentIndex])) {
