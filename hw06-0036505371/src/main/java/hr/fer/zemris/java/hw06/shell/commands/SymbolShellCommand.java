...
/**
 * Razred koji modelira naredbu za ispisivanje trenutnog znaka za
 * <code>PROMPT</code>, <code>MORELINES</code> ili <code>MULTILINE</code>.
...
public class SymbolShellCommand implements ShellCommand {
	...
	public SymbolShellCommand() {
		...
		commandDescription.add("Symbol command is used to get or set symbols for PROMPT, MORELINES "
				+ "or MULTILINE.");
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		if(args.length == 1) {
			...
			if(args[0].equals(PROMPT)) {
				...
				env.writeln(
						"Symbol for MORELINES changed from '" + env.getMorelinesSymbol() 
						+ "' to '" + newSymbol + "'.");
				env.setMorelinesSymbol(newSymbol);
				
			} else if(args[0].equals(MULTILINE)) {
				env.writeln(
						"Symbol for MULTILINE changed from '" + env.getMultilineSymbol() 
						+ "' to '" + newSymbol + "'.");
