...
/**
 * Ovaj razred nudi pomoćne statičke metode za pretvorbu broja iz heksadekadskog
 * zapisa u bajtni zapis i obratno.
 * 
 * @author Maja Radočaj
 *
 */
public class Util {
	 ...
	 * Zapis se pretvara u niz bajtova koji se na kraju vraća u obliku polja
	 * bajtova. Ukoliko primljeni argument nije valjanog oblika, baca se iznimka.
	 * 
	 * @param keyText heksadekadski zapis
	 * @return bajtni zapis
	 * @throws IllegalArgumentException ako primljeni argument nije valjani
	 *                                  heksadekadski zapis
	...
	public static byte[] hextobyte(String keyText) {
		...
		if(text.length() % 2 != 0) {
			throw new IllegalArgumentException("Invalid argument. Length of given " + 
					"argument shouldn't be odd.");
		}
		...
		for(int i = 0; i < characters.length; i = i + 2) {
			...
			result[i / 2] = (byte) ((Character.digit(characters[i], 16) << 4) + 
					Character.digit(characters[i + 1], 16));
		}
	...
	/**
	 * Pomoćna metoda koja provjerava je li trenutni broj validni heksadekadski
	 * zapis jednog bajta.
	 * 
	 * @param first  prva znamenka broja
	 * @param second druga znamenka broja
	 * @return <code>true</code> ako su znamenke validne (0-9 i A-F),
	 *         <code>false</code> ako nisu
	 */
	private static boolean isValidHex(char first, char second) {
		if(!Character.isDigit(first) && !(first >= 'a' && first <= 'f'))
			return false;
		if(!Character.isDigit(second) && !(second >= 'a' && second <= 'f'))
			return false;
		return true;
	}
