...
/**
 * Program koji omogućava pokretanje jednostavne animacije - rotiranje oko nekih
 * grafičkih objekata u sceni.
 * 
 * @author Maja Radočaj
 *
 */
public class RayCasterParallel2 {
	...
	/**
	 * Metoda koja vraća objekt koji animira scenu.
	 * 
	 * @return objekt koji može animirati scenu
	 */
...
//				Point3D zAxis = ...;
				...
				pool.invoke(new RayTracerJob(0, height - 1, width, height, vertical, 
						horizontal, xAxis, yAxis, scene,
						red, green, blue, screenCorner, maxIter, eye));
				pool.shutdown();
	...
	private static class RayTracerJob extends RecursiveAction {
		...
		protected void compute() {
			...
			invokeAll(
					new RayTracerJob(yMin, yMin + (yMax - yMin) / 2, width, height, 
							vertical, horizontal, xAxis, yAxis,
							scene, red, green, blue, screenCorner, maxIter, eye),
					new RayTracerJob(yMin + (yMax - yMin) / 2 + 1, yMax, width, height, 
							vertical, horizontal, xAxis,
	...
	private static short[] determineColorFor(RayIntersection closestIntersection, Scene scene, Ray ray) {
		...
		for(LightSource ls : scene.getLights()) {
			Ray newRay = new Ray(ls.getPoint(), closestIntersection.getPoint().sub(
					ls.getPoint()).modifyNormalize());
